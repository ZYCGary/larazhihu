    ...
    <li class="media" name="answer{{ $answer->id }}" id="answer{{ $answer->id }}">
        ...
        <div class="media-body">
            <div class="media-heading mt-0 mb-1 text-secondary">
                ...
                <a class="float-right" >
                    @if($answer->isBest())
                        <button class="btn btn-warning btn-sm"><i class="fa fa-check"></i> Best Answer</button>
                    @else
                        @can('update', $question)
                            <form action="/answers/{{ $answer->id }}/best" method="POST" accept-charset="UTF-8">
                                {{ csrf_field() }}
                                <button type="submit" class="btn btn-primary btn-sm">Set As The Best Answer</button>
                            </form>
                        @endcan
